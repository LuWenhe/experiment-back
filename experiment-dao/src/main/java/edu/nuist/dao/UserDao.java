...
public interface UserDao {
    ...
    @Insert("insert into users (username,password,realName,phone,email,created_time,role,avatar_image) " +
            "values (#{username},#{password},#{realName},#{phone},#{email},#{created_time},1,#{avatar_image})")
    void insertUserFromExcel(UserExcel userExcel);

    @Insert("insert into users (username,password,realName,phone,email,created_time,role,avatar_image) " +
            "values (#{username},#{password},#{realName},#{phone},#{email},#{created_time},2,#{avatar_image})")
